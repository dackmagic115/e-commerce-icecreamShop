                        ...
                        <ul class="nav navbar-nav navbar-right" >
                            ...
                            <a href="#" class=""  aria-haspopup="true" style=" color: #fff;">ยินดีต้อนรับคุณ <?php echo $_SESSION['fname']; ?>  <i class="caret"></i></a>
                              <ul class="dropdown-menu" style="font-size: 20px;">
                                ...
                                <li><a href="c_checkorder.php" >ประวัติการสั่งซื้อ</a></li>
                                <li><a href="logout.php">ออกจากระบบ</a></li>
                              </ul>
                            </li>
                          </ul>
    ...
    <section id="feature" style="background: #444; padding-top: 150px; height: 100%;">
        <div style="padding-left: 0; padding-right: 0;" align="center">
           <div class="center wow fadeInDown">
                <h2 style="text-shadow: 4px 4px 2px 0px black;">รายการใบสั่งซื้อ</h2>
            </div>
              <div class="row">
            ...
            <?php
                    $sql="SELECT count(*) as total from tb_order ";
                        ...
                        $start=($P-1)*$RPP;
                        $sql="SELECT  *  FROM tb_order WHERE status = 1 limit  $start, $RPP";
                        $result = mysqli_query($conn, $sql);
                        $count=0;
        ...
        <tr bgcolor="<?=$bg?>">
            ...
            <td> 
                ...
                <input type="button" name="see_details" class="btn btn-secondary" value="ดูรายละเอียด" onclick="window.location='adminSeeOrderdetil.php?id=<?=$row["ID_order"]?>'">
               
                 <a href="admindeleteorder.php?id=<?=$row["ID_order"]?>"  name="delete_order" class="btn btn-warning" onclick="return confirm('คุณต้องการยกเลิกรายการสินค้านี้หรือไม่?')" >ยกเลิก</a>
         ...
         <div class="row" style="padding-top: 300px">
            ...
            <?php
                    $sql="SELECT count(*) as total from tb_order ";
                        ...
                        $start=($P2-1)*$RPP;
                        $sql="SELECT  *  FROM tb_order WHERE status = 0   limit  $start, $RPP";
                        $result = mysqli_query($conn, $sql);
                        $count=0;
        while( $row = mysqli_fetch_assoc($result))
            {
